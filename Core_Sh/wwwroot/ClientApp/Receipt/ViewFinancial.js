...
(function (ViewFinancial) {
    ...
    function _SearchBox_Change() {
        ...
        //	let SearchDetails = _UsersList.filter(x => x.RefNo?.toLowerCase().search(search) >= 0 || x.Amount?.toString().toLowerCase().search(search) >= 0);
    ...
    function ViewUser(item) {
        ...
        SetModelGlopel(item);
        if (item.TrType == 0) {
            OpenPagePartial("Receipt", "EditReceipt", () => { Display_Refrsh(); });
        }
        else {
            OpenPagePartial("Payment", "EditReceipt", () => { Display_Refrsh(); });
        }
    }
    function GetData() {
    ...
    function Print_Pdf(TrID, Trtype) {
        ...
        if (Trtype == 0) { // Summary
            if (Res.Lang == "Ar") {
                ...
                Print_Report("Prnt_FinancialRecieptEn", "IProc_Prnt_Financial", RepParam);
            }
        }
        else {
            if (Res.Lang == "Ar") {
                ...
                Print_Report("Prnt_FinancialExpenseEn", "IProc_Prnt_Financial", RepParam);
            }
        }
    }
