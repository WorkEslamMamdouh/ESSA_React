...
(function (Purchases) {
    ...
    var ModelMaster = new I_TR_Purchases();
    ...
    var BtnSearchPurOrder;
    var BackTap;
    var NextTap;
    var SaveUpdate;
    var BtnNameSupplier;
    var BtnCancelSupplier;
    var PayAmount;
    var Txt_CardPrc;
    var VatTypePrc;
    var Txt_TrNo;
    ...
    function InitalizeControls() {
        BtnSearchPurOrder = document.getElementById('BtnSearchPurOrder');
        ...
        BtnNameSupplier = document.getElementById('BtnNameSupplier');
        BtnCancelSupplier = document.getElementById('BtnCancelSupplier');
    ...
    function InitializeEvents() {
        BtnSearchPurOrder.onclick = BtnSearchPurOrder_onclick;
    ...
    function BtnSearchPurOrder_onclick() {
        //sys.FindKey("PurOrder", "PurOrderNo", " CompCode = " + CompCode + "  and Status = 1  and  TrType =  2", () => {
        //sys.FindKeySpeed("Pur", " CompCode = " + CompCode + "  and Status = 1  and  TrType =  2 ", 'SearchForm', () => {
        sys.FindKeyPagination("Pur", "Pur", " CompCode = " + CompCode + "  and Status = 1  and  TrType =  2 ", () => {
            let DataRow = SelectDataSearch.DataRow;
            $('#PurOrderNo').val(DataRow.TrNo);
            $('#PurOrderID').val(DataRow.PurchaseID);
            Display_PurOrder(DataRow.PurchaseID);
        });
    }
    function Display_PurOrder(PurchaseID, IsJobOrd = false) {
        let data = GetDataFrom("IQ_TR_Purchases", " PurchaseID =  " + PurchaseID);
        if (data.length > 0) {
            if (IsNew) {
    ...
    function InitializeGrid() {
        ...
        Grid.Column = [
            ...
            { title: Res.Lang == "Ar" ? "الاجمالي" : "Total", ID: "ItemTotal", Name: "ItemTotal", Type: "number", value: "", class: "_IsReturnDis", style: "width: 100%", Edit: false, visible: true, Validation: Valid.Set(false), ColumnType: ControlType.Input() },
    ...
    function SearchItems() {
        ...
        sys.FindKeyPagination("Items", "btnItems", " CompCode = " + CompCode + "  and ISActive = 1  " + WhereConRedis + "   and CatID = " + CatID + " and ItemFamilyID = " + ItemFamilyID, () => {
            ...
            ('UnitPrice').Set_Val(DataRow.CostPrice.toString(), Grid);
            ('Quantity').Set_Val("1", Grid);
            //ItemUnits = _IQ_GetItemInfo.filter(x => x.ItemID == DataRow.ItemID && (x.TypeUsing == 0 || x.TypeUsing == 2))
    ...
    function SearchSupplier() {
        ...
        sys.FindKeyPagination("Supplier", "btnSupplier", " CompCode = " + CompCode + "  and Isactive = 1  ", () => {
            let DataRow = SelectDataSearch.DataRow;
            ValidSupplEnter();
            $('#SupplierID').val(DataRow.SupplierID);
            $('#SupplierName').val(DataRow.SupplierName);
            $('#Mobile').val(DataRow.Mobile);
        });
    }
    function FunNextTap() {
        if (!Validation()) {
    ...
    function Assign() {
        ...
        ModelMaster.PurchaseID = IsReturn == true ? 0 : GlopID;
