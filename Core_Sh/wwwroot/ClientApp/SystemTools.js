class SystemTools {
    ...
    GetResourceByName(callbackfn) {
        let func = callbackfn.toString().split(".")[1].split(";")[0];
    ...
    FindKey(moduleCode, _SearchControlName, Condition, OnSearchSelected) {
        debugger;
        ...
        Ajax.CallAsync({
            ...
            success: (resp) => {
                ...
                let boxWidth = settings.Width <= 100 ? "90%" : settings.Width.toString() + "px";
                let boxHeight = settings.Height <= 100 ? "70%" : settings.Height.toString() + "px";
                let boxLeft = settings.Left <= 50 ? "5%" : settings.Left.toString() + "px";
                let boxTop = settings.Top <= 50 ? "10%" : settings.Top.toString() + "px";
    ...
    FindKeySpeed(SearchFormCode, Condition, NameFolder, OnSearchSelected) {
        var Privileg = GetPrivilegesByModuleCode(GetModuleCode());
        //alert(GetModuleCode())
        //alert(Privileg.VIEW)
        if (!Privileg.VIEW) {
    ...
    FindKeyPagination(moduleCode, _SearchControlName, Condition, OnSearchSelected) {
        var Privileg = GetPrivilegesByModuleCode(GetModuleCode());
        if (!Privileg.VIEW) {
            ShowMessage(" Not Privilege View 😒", " لا يوجد صلاحية العرض 😒");
            return;
        ...
        Ajax.CallAsync({
            ...
            success: (resp) => {
                ...
                debugger;
                let result = JSON.parse(response.DataResult);
                let _PaginationResult = response.PaginationResult;
                let settings = response.Settings[0]; //as G_SearchForm;
                let NameTable = response.TableName;
                let Condition = response.Condition;
                let _Grid_Pagination = new JsGrid();
    ...
    //}
    //new code//
    FindNotification(moduleCode, _SearchControlName, Condition, OnSearchSelected) {
        this.orgCondition = Condition;
        Ajax.CallAsync({
            ...
            success: (resp) => {
                ...
                SearchGrid.SearchDataGrid.PageSize = settings.PageSize; // < 50 ? 50 : settings.PageSize;
                SearchGrid.SearchDataGrid.PrimaryKey = settings.ReturnDataPropertyName; //"RowIndex";
                let boxWidth = settings.Width <= 100 ? "70%" : settings.Width.toString() + "px";
                let boxHeight = settings.Height <= 100 ? "50%" : settings.Height.toString() + "px";
                let boxLeft = settings.Left <= 50 ? "14%" : settings.Left.toString() + "px";
                let boxTop = settings.Top <= 50 ? "10%" : settings.Top.toString() + "px";
                $("#SearchBox").css("width", boxWidth);
                $("#SearchBox").css("height", boxHeight);
                $("#SearchBox").css("left", boxLeft);
    ...
    //***********************************************//
    GenerateFiltersKey(moduleCode, sh, columns, dataSource, onSuccess) {
        let SearchFilters = DocumentActions.GetElementById("SearchFilters");
        let sqlConditions = new Array();
        SearchFilters.innerHTML = "";
        let SearchFilterTypes = DocumentActions.GetElementById("SearchFilterTypes");
        SearchFilterTypes.innerHTML = "";
        for (var column of columns) {
            if (column.hidden == true)
                continue;
            let txt = DocumentActions.CreateElement("input");
            ...
            txt.onkeyup = (e) => {
                ...
                let columnKey = currentInput.id.replace("Filter_", "");
            ...
            let td = DocumentActions.CreateElement("td");
            td.appendChild(txt);
            SearchFilters.appendChild(td);
            let tdType = DocumentActions.CreateElement("td");
            tdType.appendChild(this.GenerateFilterTypes(column));
            SearchFilterTypes.appendChild(tdType);
        }
    }
    GenerateFilters(moduleCode, columns, dataSource, onSuccess) {
        let SearchFilters = DocumentActions.GetElementById("SearchFilters");
        let sqlConditions = new Array();
        SearchFilters.innerHTML = "";
        let SearchFilterTypes = DocumentActions.GetElementById("SearchFilterTypes");
    ...
    ImgPopup(CompCode, Branch, moduleCode, TrNo) {
        let opt = {
