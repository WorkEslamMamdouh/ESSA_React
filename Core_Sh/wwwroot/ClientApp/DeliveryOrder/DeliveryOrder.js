$(document).ready(() => {
    DeliveryOrder.InitalizeComponent();
});
var DeliveryOrder;
(function (DeliveryOrder) {
    ...
    DeliveryOrder.InitalizeComponent = InitalizeComponent;
    function DisplayDataUpdate(data, _GlopID, _GlopTrNo, _TrType, Is_DetailInsert = false) {
        ...
        $('#purchaseorderCode').val(data.purchaseorderCode);
    ...
    function BtnSearchJobOrder_onclick() {
        ...
        sys.FindKeyPagination("View_JobOrd", "View_JobOrd", " CompCode = " + CompCode + "  and Status = 1   ", () => {
            let SelectedItem = SelectDataSearch.DataRow;
            //let SelectedItem: I_TR_Sales = SearchGrid.SearchDataGrid.SelectedItem;
            $('#JobOrderNo').val(SelectedItem.TrNo);
            $('#JobOrderID').val(SelectedItem.SaleID);
            Display_ShowPriceOrJobOrder(SelectedItem.SaleID, true);
        });
    }
    function Display_ShowPriceOrJobOrder(SaleID, IsJobOrd = false) {
        ...
        if (data.length > 0) {
            if (IsNew) {
                DisplayDataUpdate(data[0], 0, 0, 4, true);
                NameFunction = "Insert";
            }
            else {
                DeleteAllDetail();
                DisplayDataUpdate(data[0], GlopID, GlopTrNo, 4, true);
                NameFunction = "Update";
            }
            if (IsJobOrd) {
    ...
    function InitializeGrid() {
        ...
        Grid.Column = [
            ...
            { title: "Cat_________", ID: "CatID", Name: "CatID", Type: "text", value: "", class: "  _IsReturnDis", style: "width: 100%", Edit: true, visible: ISHidenService, Validation: Valid.Set(true, 'Must Select Category', "#CatID#" + "==" + "-1"), ColumnType: ControlType.Dropdown(_Category, "CatID", "DescA", "Select", CatChange) },
            { title: "ItemFamily_________", ID: "ItemFamilyID", Name: "ItemFamilyID", Type: "text", value: "", class: "  _IsReturnDis", style: "width: 100%", Edit: true, visible: ISHidenService, Validation: Valid.Set(true, 'Must Select ItemFamily', "#ItemFamilyID#" + "==" + "-1"), ColumnType: ControlType.Dropdown([], null, "Select") },
            ...
            { title: "Price_________", ID: "UnitPrice", Name: "UnitPrice", Type: "number", value: "", class: "_IsReturnDis", style: "width: 100%", Edit: true, visible: false, Validation: Valid.Set(false), ColumnType: ControlType.Input(ComputeTotalsRow, ComputeTotalsRow) },
            ...
            { title: "DiscAmount_________", ID: "DiscountAmount", Name: "DiscountAmount", Type: "number", value: "", class: "_IsReturnDis", style: "width: 100%", Edit: true, visible: false, Validation: Valid.Set(false), ColumnType: ControlType.Input(() => { ComputeTotalsRow(true); }, () => { ComputeTotalsRow(true); }) },
            { title: "NetUnitPrice_________", ID: "NetUnitPrice", Name: "NetUnitPrice", Type: "number", value: "", class: "_IsReturnDis", style: "width: 100%", Edit: false, visible: false, Validation: Valid.Set(false), ColumnType: ControlType.Input(CompletOneHandQuantity, CompletOneHandQuantity) },
            { title: "VatPrc_%_______", ID: "VatPrc", Name: "VatPrc", Type: "number", value: VatTypePrc.value, class: "_IsReturnDis", style: "width: 100%", Edit: false, visible: false, Validation: Valid.Set(false), ColumnType: ControlType.Input() },
            { title: "Total_________", ID: "ItemTotal", Name: "ItemTotal", Type: "number", value: "", class: "_IsReturnDis", style: "width: 100%", Edit: false, visible: false, Validation: Valid.Set(false), ColumnType: ControlType.Input() },
            { title: "VatAmount_________", ID: "VatAmount", Name: "VatAmount", Type: "number", value: "", class: "_IsReturnDis", style: "width: 100%", Edit: false, visible: false, Validation: Valid.Set(false), ColumnType: ControlType.Input() },
            { title: "NetAfterVat_________", ID: "NetAfterVat", Name: "NetAfterVat", Type: "number", value: "", class: "_IsReturnDis", style: "width: 100%", Edit: false, visible: false, Validation: Valid.Set(false), ColumnType: ControlType.Input() },
    ...
    function Validation() {
        //if ($('#ReNo').val().trim() == '') {
        //    Errorinput($('#ReNo'), 'Must Enter PO.NO 😒', 'يجب إدخال رقم PO 😒')
        //    return false
        //}
        if (CompCode != 7) {
        ...
        if (Is_CarCenter == false) {
            if ($('#purchaseorderCode').val().trim() == "") {
                Errorinput($('#purchaseorderCode'), 'Po.No Code 😒', 'يجب إدخال كود   😒');
                return false;
            }
        }
        if (Is_CarCenter) {
